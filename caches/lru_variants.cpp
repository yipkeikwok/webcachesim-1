...
AdaptSizeS4LRUCache::AdaptSizeS4LRUCache()
    : S4LRUCache()
    , _cParam(1 << 15)
    , statSize(0)
    , _maxIterations(15)
...
void AdaptSizeS4LRUCache::setPar(std::string parName, std::string parValue) {
    if(parName.compare("t") == 0) {
        const uint64_t t = stoull(parValue);
        assert(t>1);
        _reconfiguration_interval = t;
...
bool AdaptSizeS4LRUCache::lookup(SimpleRequest* req)
{
    reconfigure(); 
    ...
    */
    ...
    return S4LRUCache::lookup(req);
}

void AdaptSizeS4LRUCache::admit(SimpleRequest* req)
{
    double roll = _uniform_real_distribution(globalGenerator);
    double admitProb = std::exp(-1.0 * double(req->getSize())/_cParam); 
...
void AdaptSizeS4LRUCache::reconfigure() {
    --_nextReconfiguration;
    if (_nextReconfiguration > 0) {
        return;
    } else if(statSize <= getSize()*3) {
...
double AdaptSizeS4LRUCache::modelHitRate(double log2c) {
    // this code is adapted from the AdaptSize git repo
    // github.com/dasebe/AdaptSize
    double old_T, the_T, the_C;
    double sum_val = 0.;
